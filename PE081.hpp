...
int PE081() {
	...
	function<int(vector<vector<int>>)> dijkstra2DArrayVer1 = [] (vector<vector<int>> grid) {
		struct cell {
			int row, col, val;
			cell() {}
		...
		while (pq.size()) {
			...
			for (pair<int, int> move: moves) {
				...
				if (nextRow < n && nextCol < m && (dist[nextRow][nextCol] == -1 || cur.val + grid[nextRow][nextCol] < dist[nextRow][nextCol])) {
	...
	cin.clear();
	return dijkstra2DArrayVer1(grid);
}